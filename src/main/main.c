...
int main(int argc, char *argv[])
{
    ...
    if (algorithmId == 0)
        ...
        sprintf(algorithmName, "%s", "MCOP_A (diagonal by diagonal)");
    else if (algorithmId == 2)
        sprintf(algorithmName, "%s", "MCOP_B (pixel by pixel)");
    else if (algorithmId == 3)
        sprintf(algorithmName, "%s", "MCOP_C (Kechid et Myoupo PDPTA'09)");
    else if (algorithmId == 4)
        sprintf(algorithmName, "%s", "MCOP_D (Lacmou et Kengne PDPTA'18)");
    else if (algorithmId == 5)
        sprintf(algorithmName, "%s", "MCOP_E (4s)");
    else
    {
    ...
    if (algorithmId == 0)
    {
        ...
        {
            startTime = MPI_Wtime();
            mcop = godboleAlgorithm();
            endTime = MPI_Wtime();
            totalTime = endTime - startTime;
            calculTime = totalTime;
            calculRate = 100;

            outputInfo.mcop = mcop;
